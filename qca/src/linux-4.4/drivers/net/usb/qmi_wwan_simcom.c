 ...
 */
#define VERSION_NUMBER "V1.0.10.28"
#define SIMCOM_WWAN_VERSION "Simcom_Linux&Android_QMI_WWAN_Driver_"VERSION_NUMBER
static const char driver_name[] = "qmi_wwan_simcom";
...
*/
#define SIMCOM_WWAN_QMAP 4 //MAX is 7

#if defined(SIMCOM_WWAN_QMAP)
#define SIMCOM_QMAP_MUX_ID 0x81
...
#define SIMCOM_BRIDGE_MODE
#endif

#ifdef SIMCOM_BRIDGE_MODE
static uint __read_mostly bridge_mode = 0/*|BIT(1)*/;
module_param( bridge_mode, uint, S_IRUGO );
#endif

#if defined(SIMCOM_WWAN_QMAP)
#define SIMCOM_UL_DATA_AGG 1

#if defined(SIMCOM_UL_DATA_AGG)
struct tx_agg_ctx {
...
typedef struct sQmiWwanQmap
{
	...
	atomic_t refcount;
	struct net_device *mpQmapNetDev[SIMCOM_WWAN_QMAP];
	uint link_state;
	uint qmap_mode;
	uint qmap_size;
	uint qmap_version;
...
static int qmap_register_device(sQmiWwanQmap * pDev, u8 offset_id)
{
    ...
    priv->mux_id = SIMCOM_QMAP_MUX_ID + offset_id;
...
static struct sk_buff *qmap_qmi_wwan_tx_fixup(struct usbnet *dev, struct sk_buff *skb, gfp_t flags) {
	...
	if (unlikely(pQmapDev == NULL)) {
		...
		if (likely(skb)) {
			...
			if (skb) {
				add_qhdr(skb, SIMCOM_QMAP_MUX_ID);
			}
			else {
				return NULL;
			}
	...
	struct sk_buff *skb_in, struct sk_buff_head *skb_chain)
{
	...
	while (skb_in->len > sizeof(struct qmap_hdr)) {
		...
		mux_id = map_header->mux_id - SIMCOM_QMAP_MUX_ID;
...
#if defined(SIMCOM_WWAN_QMAP)// && defined(CONFIG_ANDROID)
	.ndo_do_ioctl = qmap_ndo_do_ioctl,
#endif
};

static void qmi_wwan_get_drvinfo(struct net_device *net, struct ethtool_drvinfo *info)
{
...
static struct ethtool_ops qmi_wwan_ethtool_ops;
...
static int qmi_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	qmi_wwan_ethtool_ops = *dev->net->ethtool_ops;
	qmi_wwan_ethtool_ops.get_drvinfo = qmi_wwan_get_drvinfo;
	dev->net->ethtool_ops = &qmi_wwan_ethtool_ops;
	...
	if (dev->driver_info->flags & FLAG_NOARP) {
		...
		dev_info(&intf->dev, "The modem %s working on RawIP mode\n", buf);
...
#if defined(SIMCOM_WWAN_QMAP)
	if (qmap_mode > SIMCOM_WWAN_QMAP)
		qmap_mode = SIMCOM_WWAN_QMAP;
	...
	if (!status)
	{
		...
		if (dev->driver_info->flags & FLAG_NOARP)
		{
			...
			u16 fw_lvl = le16_to_cpu(dev->udev->descriptor.bcdDevice);
			int qmap_v5_support = (fw_lvl == 0x414 || fw_lvl == 0x504);
			int lte_a = qmap_v5_support;
...
static const struct usb_device_id products[] = {
	{ QMI_FIXED_RAWIP_INTF(0x1e0e, 0x9001, 5, sdx55) },
	{ }					/* END */
};
			  ...
			  const struct usb_device_id *prod)
{
	...
	if (intf->cur_altsetting->desc.bInterfaceClass != 0xff) {
		dev_info(&intf->dev,  "Simcom module not qmi_wwan mode!\n");
		return -ENODEV;
	}
...
MODULE_LICENSE("GPL");
MODULE_VERSION(SIMCOM_WWAN_VERSION);
