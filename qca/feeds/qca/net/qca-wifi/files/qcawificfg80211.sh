...
detect_qcawificfg80211() {
	...
	for dev in $(ls -d wifi* 2>&-); do
		...
		if [ "$devidx" == "0" ]; then
			mac_addr=$(macaddr_add $lan_mac 1)
		elif [ "$devidx" == "1" ]; then
			mac_addr=$lan_mac
		else
			mac_addr=$(macaddr_add $lan_mac $devidx)
		fi
	...
	if [ $( slkmac | grep "00:00:00:00:00:00" | wc -l ) -eq 0 ];then
...
config wifi-iface wlan$devidx
	option device	wifi$devidx
	option network	lan
	option mode	ap
	option ssid	SLK-Router_$getmac-$wifimac
