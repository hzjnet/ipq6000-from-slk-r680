    ...
    if (pResponse) {
        ...
        if (!QMUXResult && !QMUXError && (QMIType == pResponse->CTLMsg.GetClientIdRsp.QMIType)) {
            switch (QMIType) {
                ...
                case QMUX_TYPE_WDS_IPV6: dbg_time("Get clientWDSIPV6 = %d", ClientId); break;
            ...
            if (pResponse->CTLMsg.QMICTLMsgHdrRsp.QMUXResult == 0 && pResponse->CTLMsg.QMICTLMsgHdrRsp.QMUXError == 0) {
                ...
                for (NumElements = 0; NumElements < pResponse->CTLMsg.GetVersionRsp.NumElements; NumElements++) {
                    if (pResponse->CTLMsg.GetVersionRsp.TypeVersion[NumElements].QMUXType == QMUX_TYPE_WDA)
                        if ((pResponse->CTLMsg.GetVersionRsp.TypeVersion[NumElements].MajorVersion >= 1) &&
                            (pResponse->CTLMsg.GetVersionRsp.TypeVersion[NumElements].MinorVersion >= 19))
                        {
                            profile->wdsEnableUlParams = TRUE;
                        }
