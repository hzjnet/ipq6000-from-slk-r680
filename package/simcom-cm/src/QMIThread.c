...
void PrintIPAddr(const char *msg, unsigned int addr)
{
   dbg_time("%s : %d.%d.%d.%d",
        msg,
        addr >> 24,
...
void PrintIPV6Addr(const char *msg,  UCHAR  IPV6Address[16])
{
    char str[40];
    unsigned char *addr = (unsigned char *)IPV6Address;
...
static USHORT WdaSetDataFormat(PQMUX_MSG pMUXMsg, void *arg) {
    ...
    if (qmap_settings->rx_urb_size == 0) {
        ...
        linkProto->Value = cpu_to_le32(0x02);    /* Set RawIP  mode */
    ...
    //Maximum number of datagrams in a single aggregated packet on downlink
        ...
        pMUXMsg->SetDataFormatReq.DownlinkDataAggregationMaxDatagramsTlv.Value = cpu_to_le32(qmap_settings->rx_urb_size/1024);
    ...
    //Peripheral End Point ID
        ...
        dbg_time("Downlink data aggregation max datagrams: %d", qmap_settings->rx_urb_size/1024);
        dbg_time("usbnet.rx_urb_size: %d", qmap_settings->rx_urb_size);
...
// 2 ?C PDP-IPv6
// 3 ?C PDP-IPv4v6
    if (profile->isIpv4 && profile->isIpv6)
        pPdpType->PdpType = 3;
    else if (profile->isIpv6)
        pPdpType->PdpType = 2;
    else
        pPdpType->PdpType = 0;
...
void QmiThreadRecvQMI(PQCQMIMSG pResponse) {
    ...
    if (s_pRequest && is_response(s_pRequest, pResponse)) {
        ...
        // dbg_time("QMINAS_SERVING_SYSTEM_IND");
    } else if ((pResponse->QMIHdr.QMIType == QMUX_TYPE_WDS || pResponse->QMIHdr.QMIType == QMUX_TYPE_WDS_IPV6)
...
int requestSetEthMode(PROFILE_T *profile) {
    ...
    memset(&qmap_settings, 0, sizeof(QMAP_SETTING));
    ...
    if (linkProto != NULL) {
        if (le32_to_cpu(linkProto->Value) != 2) {
            dbg_time("EFAULT: Data Format Cannot be set to RawIP Mode\n");
        }
    ...
    if (linkProto != NULL && profile->qmap_mode) {
        ...
        dbg_time("qmiwda.dl_data_aggregation_max_size = %u", qmap_settings.rx_urb_size); //must same as rx_urb_size defined in GobiNet&qmi_wwan driver
    }
    ...
    if (qmap_settings.ul_data_aggregation_max_datagrams)
    {
        ...
        if (qmap_settings.ul_data_aggregation_max_datagrams > 1) {
            set_driver_ul_params_setting(profile, &qmap_settings);
        }
    }
#endif
    ...
    *pPSAttachedState = 0;
    ...
    while (remainingLen > 0) {
        ...
        case 0x4E: /*  Additional LTE System Info - Availability of Dual connectivity of E-UTRA with NR5G */
            ...
            // dbg_time("(0x4E endc_available %d)\n" ,endc_available);
            break;
...
int requestGetIPAddress(PROFILE_T *profile, int curIpFamily) {
    ...
    if (pIpv4Addr) {
        ...
        // PrintIPAddr("Gateway : ", pIpv4Addr->IPV4Address);
    }
    ...
    if (pIpv4Addr) {
        ...
        // PrintIPAddr("SubnetMask : ", pIpv4Addr->IPV4Address);
    }
    ...
    if (pIpv4Addr) {
        ...
        // PrintIPAddr("IPV4addr : ", pIpv4Addr->IPV4Address);
    }
    ...
    if (pIpv6Addr) {
        ...
        // PrintIPV6Addr("PDNS : ", pIpv6Addr->IPV6Address);
    }
    ...
    if (pIpv6Addr) {
        ...
        // PrintIPV6Addr("SDNS : ", pIpv6Addr->IPV6Address);
    }
    ...
    if (pIpv6Addr) {
        ...
        // PrintIPV6Addr("IPV6_Addr : ", pIpv6Addr->IPV6Address);
    }
...
int requestBaseBandVersion(const char **pp_reversion) {
    ...
    if (revId && le16_to_cpu(revId->TLVLength))
    {
        ...
        if (strstr(DeviceRevisionID, "SDX55")) {
            s_sdx55 = 1;
        }
        if (s_sdx55==1)
        {
