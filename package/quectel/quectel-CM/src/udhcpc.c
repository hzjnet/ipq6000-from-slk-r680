...
#define HOTPLUG_PATH "/sbin/hotplug-call"

static void openwrt_hotplug(const char *ifname, const char* action)
{
    char *argv[3];
    int pid;
    ...
    pid = fork();
    if (pid < 0) {
        dbg_time("hotplug_event fork failed!");
        return;
    } else if (pid == 0) {
...
void udhcpc_stop(PROFILE_T *profile) {
    ...
    // openwrt
    if (1) {
        openwrt_hotplug(ifname, "linkdown");
        return;
    }
